 ...
 * @param2 packagename
 */
public class ModelGenerateSqlServer extends ModelGenerate {

	/**
	 * 生成一个指定表的po类的java文件，适应mssql
	 * 
	 * @param tableName
	 * @param packagename
	 * @param con
	...
	public boolean createModel(String tablename, String typePack, String enc, Connection con) throws Exception {
		...
		int colcount = 0;
		try {
			String sql = "select count(*) from syscolumns where  syscolumns.id  =  object_id ('" + tablename + "') ";
			...
			sql = "  SELECT syscolumns.name AS[COLUMN_NAME],systypes.name AS [COLUMN_TYPE]"
			...
			rs = con.createStatement()
					.executeQuery(
							"SELECT  syscolumns.name COLUMN_NAME,systypes.name DATA_TYPE FROM  syscolumns, systypes WHERE  syscolumns.xusertype  =  systypes.xusertype AND  syscolumns.id  =   object_id ( '["
									+ tablename + "]' )  order by syscolumns.id ");
			int colnum = 0;
			while (rs.next()) {
			...
			File f = new File(path + "\\" + tablename + "PO.java");
			...
			for (int i = 0; i < colcount; i++) {
				if (columntype[i].equalsIgnoreCase("double") || columntype[i].equalsIgnoreCase("decimal")) {
					sb.append("\t\tattrs.put(\"" + columnname[i] + "\", java.math.BigDecimal.class);");
				} else if (columntype[i].equalsIgnoreCase("int")) {
			...
			for (int i = 0; i < colcount; i++) {
				...
				// set
				if (columntype[i].equalsIgnoreCase("double") || columntype[i].equalsIgnoreCase("decimal")) {
					sb.append("\tpublic void set" + DBUtil.firstToUpperCase(columnname[i]) + "(java.math.BigDecimal "
							+ columnname[i] + ") {\n");
				} else if (columntype[i].equalsIgnoreCase("int")) {
					sb.append("\tpublic void set" + DBUtil.firstToUpperCase(columnname[i]) + "(int " + columnname[i]
							+ ") {\n");
				} else if (columntype[i].equalsIgnoreCase("date")) {
